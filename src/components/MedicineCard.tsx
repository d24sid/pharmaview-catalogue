...
const baseBadgeClasses =
  "text-xs font-semibold mr-2 px-2.5 py-0.5 rounded-full inline-flex items-center";
...
const AvailabilityBadge: React.FC<{
  availability?: Medicine["availability"] | string | number;
}> = ({ availability }) => {
  // accept either "In Stock"/"Low Stock"/"Out of Stock" or numeric stock
  if (typeof availability === "number") {
    if (availability > 10) {
      return (
        <span className={`bg-green-100 text-green-800 ${baseBadgeClasses}`}>
    ...
    if (availability > 0) {
      return (
        <span className={`bg-yellow-100 text-yellow-800 ${baseBadgeClasses}`}>
    ...
    return (
      <span className={`bg-red-100 text-red-800 ${baseBadgeClasses}`}>
        <AlertTriangle className="w-3 h-3 mr-1" />
        Out of Stock
      </span>
    );
...
const MedicineCard: React.FC<MedicineCardProps> = ({
  ...
  const mrp =
    typeof (medicine as any).mrp === "number"
      ? (medicine as any).mrp
      : undefined;
  const hasDiscount = typeof mrp === "number" && mrp > price;

  const imageSrc =
    !imgError && medicine.imageUrl ? medicine.imageUrl : fallbackImageUrl;
        ...
        >
          <div className="w-full h-44 md:h-full bg-slate-100 overflow-hidden relative">
            ...
            <div className="absolute top-2 left-2">
              <AvailabilityBadge
                availability={medicine.availability ?? medicine.stock}
              />
            </div>
          </div>
        </Link>
          ...
          <div>
            <div className="flex justify-between items-start gap-4">
              ...
              >
                <h3 className="text-xl font-bold text-primary-dark hover:underline truncate">
                  {searchHighlight
                    ? highlight(medicine.name, searchHighlight)
                    : medicine.name}
                </h3>
                <p className="text-sm text-subtle truncate">
                  {searchHighlight
                    ? highlight(safe(medicine.genericName), searchHighlight)
              ...
              <div className="flex flex-col items-end space-y-2">
                <div className="flex items-center space-x-2">
                  ...
                  {hasDiscount && (
                    <div className="text-sm line-through text-subtle">
                      {currency(mrp!)}
                    </div>
                  )}
                </div>
            ...
            <p className="text-sm text-gray-600 mt-2">
              ...
              <span className="font-semibold">
                {searchHighlight
                  ? highlight(safe(medicine.brand), searchHighlight)
                  : medicine.brand}
              </span>
            </p>
            <div className="mt-3 flex gap-2 flex-wrap items-center">
              ...
              {/* {medicine. !== undefined && <span className="text-xs bg-slate-50 px-2 py-1 rounded">GST {medicine.gst}%</span>} */}
            </div>
          </div>
        </div>
      </article>
