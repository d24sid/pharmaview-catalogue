import { Availability, Medicine } from "./types";
...
const toBoolean = (value: any): boolean => {
  ...
  if (typeof value === "string") {
    ...
    return (
      lower === "y" || lower === "yes" || lower === "true" || lower === "1"
    );
  }
  return !!value;
};
...
const fromSheetRow = (row: Record<string, any>, index: number): Medicine => {
  const medicine: Partial<Medicine> & { raw: Record<string, any> } = {
    raw: row,
  };
  ...
  return {
    id:
      ...
      `${normalizeHeader(medicine.name || `med-${index}`)}`,
